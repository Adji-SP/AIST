...
class SerialCommunicator {
    ...
    async connect() {
        ...
        try {
            ...
            alert.serial.error('Connection failed', error);
    ...
    _startPortScanning() {
        ...
        this.portScanTimer = setInterval(async () => {
            try {
                ...
                alert.serial.error('Port scanning', error);
            }
        }, this.config.portScanInterval);

        alert.debug('SERIAL', `Port scanning started (interval: ${this.config.portScanInterval}ms)`);
    }
    ...
    _stopPortScanning() {
        if (this.portScanTimer) {
            ...
            alert.debug('SERIAL', 'Port scanning stopped');
        }
    }
    ...
    async _scanForBetterPort() {
        ...
        try {
            alert.debug('SERIAL', 'Scanning for better Arduino/ESP32 ports...');
            ...
            if (potentialPorts.length > 0) {
                ...
                if (bestPotentialPort !== this.currentPortPath) {
                    alert.serial.portDetected(bestPotentialPort, `Better than current: ${this.currentPortPath}`);
    ...
    async _switchToPort(newPortPath) {
        try {
            alert.serial.portSwitched(this.currentPortPath, newPortPath);
    ...
    async forceReconnect() {
        alert.info('SERIAL', 'Force reconnection requested');
    ...
    async disconnect() {
        alert.info('SERIAL', 'Intentional disconnect requested');
    ...
    async _autoDetectAndConnect() {
        try {
            alert.debug('SERIAL', 'Scanning for Arduino/ESP32 devices...');
            const ports = await SerialPort.list();
    ...
    async _connectToPort(portPath) {
        return new Promise((resolve, reject) => {
            alert.debug('SERIAL', `Attempting connection to ${portPath} @ ${this.config.baudRate} baud`);
            ...
            this.arduinoPort.on('open', () => {
                ...
                alert.serial.connected(portPath, this.config.baudRate);
            ...
            this.arduinoPort.on('error', (err) => {
                clearTimeout(connectionTimeout);
                alert.serial.error('Port communication', err);
            ...
            this.arduinoPort.on('close', () => {
                clearTimeout(connectionTimeout);
                alert.serial.disconnected(portPath);
    ...
    _handleData(rawString) {
        ...
        alert.debug('SERIAL', `Raw data: ${trimmedData}`);
