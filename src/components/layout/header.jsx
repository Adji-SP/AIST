...
import { Bell, Menu, LogOut, Shield, Cpu, Settings, ChevronRight } from 'lucide-react';
...
const Header = ({ onMenuClick, alerts = [] }) => {
  ...
  const roleBadge = role === 'superadmin' ? 'Super Admin'
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-3 w-72 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden origin-top-right transition-all animate-fade-in-up">
                    {/* User Info Header */}
                    <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center gap-3">
                      <div className="w-11 h-11 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                        <span className="text-white text-sm font-bold">{initials}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <span className={`inline-block mt-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ${roleBadgeColor}`}>
                          {roleBadge}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-1.5">
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-slate-700 rounded-lg hover:bg-slate-50 transition-colors group"
                      >
                        <Cpu className="w-4 h-4 text-slate-400 group-hover:text-green-600 transition-colors" />
                        <span className="flex-1 text-left">Manage Devices</span>
